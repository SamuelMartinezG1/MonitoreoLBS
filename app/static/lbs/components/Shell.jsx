...
function Header({ deviceName, page, crumbs }) {
  ...
  const navItems = [
    { id: 'dashboard',  label: 'Tablero',     ico: 'speedometer2', href: URLS.dashboard   || 'dashboard.html' },
    { id: 'monitoreo',  label: 'Monitoreo',   ico: 'activity',     href: URLS.monitoreo   || 'monitoreo.html' },
    { id: 'inventario', label: 'Inventario',  ico: 'diagram-3',    href: URLS.inventario  || 'inventario.html' },
    { id: 'diagnostico',label: 'Diagnóstico', ico: 'tools',        href: URLS.diagnostico || 'diagnostico.html' },
  ];
  const user = URLS.user || { initials: 'RC', name: 'R. Cárdenas' };
  ...
  return (
    <header className="app-header">
      ...
      <a className="user-chip" href={URLS.logout || '#'} title="Cerrar sesión" style={{ textDecoration: 'none' }}>
        <div className="avatar">{user.initials}</div>
        <span>{user.name}</span>
        <i className="bi bi-box-arrow-right" style={{ fontSize: 11, color: 'var(--text-dim)' }}></i>
      </a>
    </header>
  );
}
